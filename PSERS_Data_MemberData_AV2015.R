...
file_memberData <- "Data_inputs/PSERS_MemberData_AV2015.xlsx"
...
# Total Tier E and F members: 48469
 # Total member # in Tier E: 41189
 # Total member # in Tier F:  7280  
...
sum_tEF <- df_allTiers %>% filter(yos <= 3, planname == "Actives_allTiers_fillin") %>% 
  group_by(yos) %>% 
...
df_tE <- 
  ...
  mutate(nactives = ifelse(yos >= 4, 0, nactives * nact.tEF.AV/nact.tEF * share.tE),
         salary   = ifelse(yos >= 4, 0, salary),
         planname = "Actives_tE_fillin" )

df_tF <- 
  df_allTiers %>% filter(planname == "Actives_allTiers_fillin") %>% 
  mutate(nactives = ifelse(yos >= 4, 0, nactives * nact.tEF.AV/nact.tEF * share.tF),
         salary   = ifelse(yos >= 4, 0, salary) * factor.tF.sal,
         planname = "Actives_tF_fillin" )

df_tCD <- 
  df_allTiers %>% filter(planname == "Actives_allTiers_fillin") %>% 
  mutate(nactives = ifelse(yos < 4, nactives * (1 - nact.tEF.AV/nact.tEF), nactives ),
...
# Check results:
f_sum <- function(df){
    df  %>% filter(planname == "Actives_allTiers_fillin") %>% 
    #group_by(yos) %>% 
    summarise(nact = sum(nactives),
              avg_sal = sum(nactives * salary)/sum(nactives),
              avg_age = sum(nactives * age)/sum(nactives),
