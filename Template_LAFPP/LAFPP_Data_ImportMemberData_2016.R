...
# Data source: Data_inputs/LAFPP_MemberData_2016.xlsx
...
file_memberData <- "Data_inputs/LAFPP_MemberData_2016.xlsx"
...
#******************************************************
#  Calibrating initial non-actives to match 2016 data
#******************************************************
nretiree_2016 <- 8414
benRet_2016   <- 6056 
...
nBeneficiaries_2016 <- 2422
benBeneficiaries_2016 <- 4190

ret.sum <- 
  init_retirees_all %>% filter(grepl("fillin", planname)) %>% 
  summarise(n = sum(nretirees),
            benefit   = sum(nretirees * benefit/12)/sum(nretirees))

disb.sum <- 
  init_disb_all %>% filter(grepl("fillin", planname)) %>% 
  summarise(n = sum(ndisb),
            benefit   = sum(ndisb * benefit/12)/sum(ndisb))

ben.sum <- 
  init_beneficiaries_all %>% filter(grepl("fillin", planname)) %>% 
  summarise(n = sum(nbeneficiaries),
            benefit   = sum(nbeneficiaries * benefit/12)/sum(nbeneficiaries))

factor.nret   <- nretiree_2016/ret.sum$n
factor.benRet <- benRet_2016/ret.sum$benefit

factor.ndisb   <- ndisb_2016/disb.sum$n
factor.benDisb <- benDisb_2016/disb.sum$benefit

factor.nben   <- nBeneficiaries_2016/ben.sum$n
factor.benBen <- benBeneficiaries_2016/ben.sum$benefit


init_retirees_all %<>%
  mutate(nretirees = nretirees * factor.nret,
         benefit   = benefit * factor.benRet)

init_disb_all %<>%
  mutate(ndisb = ndisb * factor.ndisb,
         benefit   = benefit * factor.benDisb)

init_beneficiaries_all %<>%
  mutate(nbeneficiaries = nbeneficiaries * factor.nben,
         benefit   = benefit * factor.benBen)




# Allocate initial retirees to tier 2 and tier 5

initRet.t2pct <- 0.57
initRet.t5pct <- 1 - initRet.t2pct

init_retirees_t2 <- init_retirees_all %>% 
                    mutate(planname = sub("allTiers", "t2", planname),
                           nretirees = nretirees * initRet.t2pct)

init_retirees_t5 <- init_retirees_all %>% 
  mutate(planname = sub("allTiers", "t5", planname),
         nretirees = nretirees * initRet.t5pct)


init_retirees_all <-  expand.grid(age = 41:100, planname = paste0("Retirees_", paste0("t", 1:6, "_fillin"))) %>%
  left_join(bind_rows(init_retirees_t2, init_retirees_t5)) %>%
  mutate_each(funs(na2zero), -age, -planname)


# Assume all beneficiaries and disabled are in one tier. (Tier 5) 

tier.Init.Ben_Disb <- "t5"

init_beneficiaries_all %<>% mutate(planname = sub("allTiers", tier.Init.Ben_Disb, planname))
init_disb_all          %<>% mutate(planname = sub("allTiers", tier.Init.Ben_Disb, planname))  
...
# Caibrate to match 2016 data

nterm_2016  <- 128
nben50_2016 <- 2600

term.sum <- init_terms_all %>% 
  summarise(n = sum(nterms),
            ben50 = sum(nterms * benefit.50 / 12)/sum(nterms)) 

factor.nterm <- nterm_2016/term.sum$n
factor.ben50 <- nben50_2016/term.sum$ben50

init_terms_all %<>% mutate(nterms = nterms * factor.nterm,
                           benefit.50 = benefit.50 * factor.ben50) 
...
save(init_actives_all, init_retirees_all, init_beneficiaries_all, init_disb_all, init_terms_all,
     ...
     file = "Data_inputs/LAFPP_MemberData_2016.RData")
